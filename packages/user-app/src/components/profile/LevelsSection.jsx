...
const INITIAL_LEVELS_DATA = [
  ...
  //   description: "Ən çətin keçid - meşə arasında manevrə edərək uç.",
  //   image: maps_1,
  //   difficulty: "Çox Çətin",
  //   timesPlayed: 0,
  //   duration: 40,
    ...
    >
      ...
      <CardContent sx={{ pb: 2 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", mb: 1.5 }}>
          ...
          {level.completed && (
            <EmojiEventsIcon sx={{ color: "#FFD700", fontSize: "1.5rem", ml: 1 }} />
          )}
        </Box>
    ...
    const fetchMapStats = async () => {
      try {
        ...
        const mapNameToLevelId = {
          'baku city level': 1,
          'ghost city level': 2,
          'island level river': 3,
          'ghost city level hard': 4,
        };

        const updatedLevels = INITIAL_LEVELS_DATA.map((level) => {
          const stat = stats.find((s) => {
            const mapName = s.map_name?.toLowerCase?.();
            return s.map_id === level.id || mapNameToLevelId[mapName] === level.id;
          });
          const timesPlayed = stat ? stat.times_played : level.timesPlayed;
          const objectiveTime = stat && stat.objective_time_seconds ? stat.objective_time_seconds : 0;
          ...
          return {
            ...
            bestTime: stat && stat.best_time_seconds ? Math.round(stat.best_time_seconds) : null,
    ...
    <Box sx={{ mt: 3 }}>
      ...
      >
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 2 }}>
          ...
          <Box sx={{ textAlign: "center" }}>
            <Typography variant="h4" sx={{ fontWeight: 700 }}>
              {levelsData.reduce((acc, l) => acc + Number(l.timesPlayed || 0), 0)}
            </Typography>
...
<Grid container spacing={3} sx={{ mt: 4 }}>
     <Grid item xs={12} md={6}>
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
              <PersonIcon sx={{ color: "#64B5F6", mr: 1.5, fontSize: "1.6rem" }} />
              <Typography variant="h6" sx={{ fontWeight: 700 }}>
                Səviyyə nəticələri
              </Typography>
            </Box>
            {levelsData.map((level, index) => (
              ...
              >
                <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 0.5, color: "#E3F2FD" }}>
                  {level.title}
                </Typography>
                <Typography variant="body2" sx={{ color: "#BBDEFB" }}>
                  Ən yaxşı vaxt: <strong>{level.bestTime ? `${Math.floor(level.bestTime / 60)}:${(level.bestTime % 60).toString().padStart(2, '0')}` : 'N/A'}</strong>
                </Typography>
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
              <TrendingUpIcon sx={{ color: "#4CAF50", mr: 1.5, fontSize: "1.6rem" }} />
              <Typography variant="h6" sx={{ fontWeight: 700 }}>
                Ardıcıllıq
              </Typography>
            </Box>
            <Typography variant="h2" sx={{ fontWeight: 800, color: "#2E7D32", mb: 1 }}>
              {streak.current} gün
            </Typography>
